...
Na afronding van de eerste veldwerkperiode wordt de respons bestudeerd. Deze respons wordt uitgelezen als rechte telling per wijk uit de tijdelijke SPSS-file. De frequentietabel wordt aan het steekproefkader toegevoegd middels een left_join (zie script 03). Vervolgens wordt de nieuwe target per wijk voor de volgende veldwerkperiode geupdate aan de hand van de tekorten en overschotten per wijk. De nieuwe target wordt ook opgehoogd met de afgesproken steekproeffactor. Deze stappen worden herhaald totdat het veldwerk is afgerond. 
...
Vervolgens worden rechte tellingen per aankoopgebied (winkelgebieden in Amsterdam, locaties buiten Amsterdam, online, etc.) gemaakt van de vragen die betrekking hebben op waar de producten zijn aangeschaft  (zie script 00 en 01).
